...
public interface IamUserMapper extends BaseMapper<IamUser> {
    ...
    @Select("SELECT * FROM iam_user WHERE username = #{username} AND deleted = 0 LIMIT 1")
    IamUser selectByUsername(@Param("username") String username);

    /**
     * 根据用户名和租户ID查询用户
     */
    @Select("SELECT * FROM iam_user WHERE username = #{username} AND tenant_id = #{tenantId} AND deleted = 0 LIMIT 1")
    IamUser selectByUsernameAndTenantId(@Param("username") String username, @Param("tenantId") String tenantId);

    /**
     * 根据租户ID查询所有用户
     */
    @Select("SELECT * FROM iam_user WHERE tenant_id = #{tenantId} AND deleted = 0")
     ...
     */
    @InterceptorIgnore(tenantLine = "true")
    @Select("SELECT * FROM iam_user WHERE deleted = 0")
     ...
     */
    @Select("<script>" +
            "SELECT * FROM iam_user WHERE deleted = 0 " +
