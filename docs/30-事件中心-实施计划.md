...
### 4.1 第一阶段：核心事件（优先级：高）✅ 已完成

**时间：2周** | **实际完成时间：2024-12-31**

#### 4.1.1 API 生命周期事件
- [x] api.published - API 发布事件
- [x] api.updated - API 更新事件
- [x] api.offline - API 下线事件
- [x] api.deleted - API 删除事件

**已完成工作：**
- [x] 创建事件定义初始化 SQL 脚本
- [x] 在 API 服务中创建 ApiEventPublisher
- [x] 在 publishApi、offlineApi、deleteApi 方法中集成事件发布
- [x] 添加 Kafka 依赖到 API 服务
- [x] 创建第一阶段开发说明文档
- [x] 创建第一阶段测试指南

**订阅者实现：**
- [ ] 网关服务订阅 API 事件，实现路由自动刷新（待开发）
- [ ] 审计服务订阅 API 事件，记录操作日志（待开发）

#### 4.1.2 告警事件
- [x] alert.triggered - 告警触发事件
- [x] alert.resolved - 告警恢复事件

**已完成工作：**
- [x] 在治理服务中创建 AlertEventPublisher
- [x] 在 AlertDetectionJob 中集成告警触发事件发布
- [x] 治理服务已有 Kafka 依赖
